...
{
	if(SYS_mutex_lock((mutex_obj*)hMutex,time)==SYS_NO_ERR)
	{
		return TRUE;
	}
	return	FALSE;
...
{
	SYS_mutex_unlock((mutex_obj*)hMutex);
}
...
{
	SYS_mutex_destroy((mutex_obj*)hMutex);
}
...
GUI_SEM*	GUI_SemCreate(int init,int max)
{
	...
	sem =SYS_sem_create(init,max,NULL);
	return	(GUI_SEM*)sem;
}
...
{
	if(SYS_sem_wait((sem_obj*)hsem,time)== SYS_NO_ERR)
	{
		return TRUE;
	}
	return FALSE;
...
{
	return	SYS_get_time_count(NULL);
}
