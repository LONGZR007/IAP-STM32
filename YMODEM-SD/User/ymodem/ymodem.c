...
static ymodem_status_t y_received_status = Y_DIS;                     /* Ymodem的状态. */
static void *file_ptr = 0;                                            /* 供用户使用的指针 */
  ...
  if (y_received_status == Y_EN)
  {
    ...
    switch(header[0])
    {
      ...
      case Y_STX:
        ...
        {
          (void)y_transmit_ch(Y_ACK);
          run_status = Y_END;    // 文件传输正常结束
          y_received_status = Y_DIS;    // 正常结束禁用协议
        }
      ...
      /* Abort from host. */
      case Y_CAN:
        run_status = Y_CANCEL;         // 文件传输由上位机取消结束
        y_received_status = Y_DIS;     // 结束禁用协议
        break;
      default:
          /* 错误的头. */
        if (0 == receive_status)
    ...
    if (status == Y_ERROR)                   // 传输发生错误结束
    {
      run_status = Y_MAX_ERR;
      if (Y_ERROR_FLASH == packet_status)    // 底层错误
      {
        run_status = Y_DISK_ERR;             // 标记底层错误
      }
      y_received_status = Y_DIS;             // 禁用接收
    }
  }
  
  return y_received_status;    // 文件接收出错,返回接收到的数里和错误信息
}
...
static int get_receive_data(y_uint8_t **data, y_uint32_t len)
{
  ...
  y_uint16_t data_len[2] = {128, 1024};
    ...
    if (max_len >= 1)
    {
      ...
      {
        if (max_len >= data_len[*data_temp - 1] + 3 + 2);             // 根据不同的头记录不同的长度
        {
          /* 获取接收数据 */
          *data = recv_buf;
          reset_recv_len();
